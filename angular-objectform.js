...
directive('objectForm',['$compile',function($compile){
    return {
        ...
        template: '<table id="ObjectForm"><tr data-ng-repeat="property in parsedObject" id="ObjectFormProperty_{{property.name}}">'
                            ...
                            +'<td><input class="form-control" type="text" data-ng-model="formData[property.name]" name="{{property.name}}"/></td><td>&nbsp;<a href="" data-ng-click="removeFormProperty(property.name)"><i class="glyphicon glyphicon-remove"></i></a></td>'
